...
### Re-anclaje del comodín al agregar la carta real

Cuando un jugador agrega a una escalera exactamente la carta que el comodín está representando en ese momento, el motor aplica la regla de **re-anclaje**:

- El comodín cede su posición a la carta real y se desplaza hacia el extremo libre más cercano de la escalera (se prefiere el extremo alto cuando ambos extremos están libres, por coherencia con la convención de posicionamiento inicial).
- Si el comodín estaba en un extremo, se desplaza un paso más hacia ese mismo extremo (representa ahora la siguiente carta en esa dirección).
- Si el comodín estaba rellenando un hueco interno, se reubica en el extremo alto; si ese extremo ya está en el límite de la secuencia, se intenta el extremo bajo.
- Si el extremo al que debería desplazarse ya está en el límite natural de la secuencia (p. ej. el comodín representa el As en una escalera Q-K-JKR y alguien agrega el As real, o el comodín representa el As bajo en JKR-2-3 y alguien agrega el As real), **el agregado se rechaza** con el mensaje: *"el comodín no se puede desplazar — no hay lugar en la escalera"*.

La escalera almacenada siempre debe ser una secuencia válida leída de izquierda a derecha; el re-anclaje garantiza esa invariante después de cada agregado.
